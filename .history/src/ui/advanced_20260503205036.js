...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        ...
        if (typeof TrafficGenerator !== 'undefined' && !window.trafficGenerator) {
            ...
            if (!document.getElementById('openTrafficGenBtn')) {
                const tgBtn = document.createElement('button');
                tgBtn.className = 'rp-btn';
                tgBtn.id = 'openTrafficGenBtn';
                tgBtn.innerHTML = `<svg class="rp-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><polyline points="2,16 5,10 8,13 11,7 14,11 17,5"/><circle cx="17" cy="5" r="1.8" fill="currentColor" stroke="none"/></svg><span class="rp-label">Gen. Tráfico</span><span class="rp-desc">Generador automático</span>`;
                const monList = document.querySelector('#rpanel-monitor .rp-list');
                if (monList) monList.appendChild(tgBtn);
