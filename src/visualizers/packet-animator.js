...
window._paInit = function(sim) {
    ...
    if (!document.getElementById('openPABtn')) {
        const btn = document.createElement('button');
        btn.className = 'rp-btn';
        btn.id        = 'openPABtn';
        btn.innerHTML = `<svg class="rp-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="10" cy="10" r="3" fill="currentColor" stroke="none"/><circle cx="10" cy="10" r="7" stroke-dasharray="2 2"/><path d="M10 3v2M10 15v2M3 10h2M15 10h2" stroke-linecap="round" stroke-width="1.3"/></svg><span class="rp-label">Pkts</span><span class="rp-desc">Monitor de paquetes</span>`;
        btn.addEventListener('click', () => window.packetAnimator.toggle());
        // Insertar al inicio del grupo Monitor del rail derecho
        const monitorList = document.querySelector('#rpanel-monitor .rp-list');
        if (monitorList) monitorList.insertBefore(btn, monitorList.firstChild);
    }
