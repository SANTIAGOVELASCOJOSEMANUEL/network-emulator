...
`;
        ...
        if (!document.getElementById('openARPBtn')) {
            const btn = document.createElement('button');
            btn.className = 'rp-btn';
            btn.id        = 'openARPBtn';
            btn.innerHTML = `<svg class="rp-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="5" width="14" height="10" rx="2"/><path d="M7 9h6M7 12h4" stroke-linecap="round"/></svg><span class="rp-label">ARP</span><span class="rp-desc">Tabla ARP del dispositivo</span>`;
            btn.addEventListener('click', () => this.toggle());
            const vizList = document.querySelector('#rpanel-viz .rp-list');
            if (vizList) vizList.appendChild(btn);
        }
    }
