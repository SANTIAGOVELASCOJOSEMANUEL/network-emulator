...
`;
        ...
        if (!document.getElementById('openRVBtn')) {
            const btn = document.createElement('button');
            btn.className = 'rp-btn';
            btn.id        = 'openRVBtn';
            btn.innerHTML = `<svg class="rp-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 5h14M3 10h14M3 15h14" stroke-linecap="round"/><circle cx="7" cy="5" r="2" fill="currentColor" stroke="none"/><circle cx="13" cy="10" r="2" fill="currentColor" stroke="none"/><circle cx="7" cy="15" r="2" fill="currentColor" stroke="none"/></svg><span class="rp-label">Rutas</span><span class="rp-desc">Routing dinámico</span>`;
            btn.addEventListener('click', () => this.toggle());
            const protoList = document.querySelector('#rpanel-proto .rp-list');
            if (protoList) protoList.appendChild(btn);
        }
    }
