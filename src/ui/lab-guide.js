...
`;
        ...
        // Botón en sidebar
        if (!document.getElementById('openLabBtn')) {
            const btn = document.createElement('button');
            btn.className = 'rp-btn';
            btn.id        = 'openLabBtn';
            btn.innerHTML = `<svg class="rp-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M7 2h6v8l2 4H5l2-4V2z" stroke-linejoin="round"/><path d="M7 10h6M9 2v2M11 2v2" stroke-linecap="round"/></svg><span class="rp-label">Lab</span><span class="rp-desc">Laboratorio guiado</span>`;
            btn.addEventListener('click', () => this.toggle());
            const diagList = document.querySelector('#rpanel-diag .rp-list');
            if (diagList) diagList.appendChild(btn);
        }
