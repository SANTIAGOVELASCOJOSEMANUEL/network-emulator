...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleAdvBtn(id) {
        document.querySelectorAll('.rail-btn, .rp-btn').forEach(b => b.classList.remove('rail-on'));
        const btn = $(id); if (btn) btn.classList.add('rail-on');
        setTimeout(() => btn?.classList.remove('rail-on'), 400);
    }
    ...
    // listeners principales ya registrados arriba — sin duplicar
...
/* ── Rail derecho: toggle de palettes (espejo del sidebar de equipos) ── */
(function initRailPalettes() {
    let openRcat = null;

    function closeRailPalettes() {
        document.querySelectorAll('.rail-palette').forEach(p => p.classList.remove('open'));
        document.querySelectorAll('.rail-cat').forEach(b => b.classList.remove('active'));
        openRcat = null;
    }

    document.querySelectorAll('.rail-cat').forEach(btn => {
        btn.addEventListener('click', e => {
            e.stopPropagation();
            const cat = btn.dataset.rcat;
            const palette = document.getElementById('rpanel-' + cat);
            if (!palette) return;

            if (openRcat === cat) { closeRailPalettes(); return; }
            closeRailPalettes();
            openRcat = cat;
            btn.classList.add('active');

            // Alinear verticalmente con el botón, anclado al rail
            const rect = btn.getBoundingClientRect();
            const rail  = document.getElementById('toolsRail').getBoundingClientRect();
            palette.style.top  = (rect.top - rail.top) + 'px';
            palette.style.right = rail.width + 'px';
            palette.classList.add('open');
        });
    });

    document.addEventListener('click', e => {
        if (!e.target.closest('.tools-rail') && !e.target.closest('.rail-palette'))
            closeRailPalettes();
    });

    // Redirigir clicks de los rp-btn al mismo handler que antes usaban los rail-btn
    // Los IDs de los botones dentro de las palettes son los mismos (openCLIBtn, etc.)
    // Los listeners ya están definidos en app.js arriba, así que funcionan automáticamente.
})();